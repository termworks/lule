...
pub fn gen_main_six(col []color.Color) []color.Color {
	...
	for colors.len < 6 {
		if colors.len == 0 {
			colors << color.color_from_rgb(128, 128, 128)
			continue
		}
...
pub fn get_black_white(ac color.Color, black_mix f64, white_mix f64, theme bool) (color.Color, color.Color) {
...
pub fn get_two_grays(ac color.Color, mix f64, theme bool) (color.Color, color.Color) {
	darker := color.color_from_rgb(100, 100, 100)
	lighter := color.color_from_rgb(170, 170, 170)
...
pub fn gen_prime_six(colors []color.Color, mix f64, theme bool) []color.Color {
	mut out := []color.Color{}
	for col in colors {
...
pub fn gen_second_six(colors []color.Color, mix f64, theme bool) []color.Color {
	mut out := []color.Color{}
	for col in colors {
		out << if !theme { col.lighten(mix) } else { col.darken(mix) }
	}
...
pub fn gen_shades(colors []color.Color, number int) []color.Color {
	mut gradients := []color.Color{}
	if colors.len < 2 {
		return gradients
	}
	count := number + 2
...
fn sample_scale(stops []color.Color, position f64) color.Color {
	if position <= 0.0 {
		return stops[0]
	}
	if position >= 1.0 {
...
pub fn gen_gradients(ac color.Color, col0 color.Color, col15 color.Color, black color.Color, white color.Color) []color.Color {
	mut gradients := []color.Color{}
	gradients << black
...
pub fn generate_defined_colors(theme bool) []color.Color {
	base_colors := [
		color.color_from_rgb(228, 228, 228),
		color.color_from_rgb(148, 148, 148),
		color.color_from_rgb(198, 198, 132),
		color.color_from_rgb(227, 199, 138),
		color.color_from_rgb(238, 239, 159),
		color.color_from_rgb(222, 147, 95),
		color.color_from_rgb(240, 143, 133),
		color.color_from_rgb(225, 150, 162),
		color.color_from_rgb(255, 203, 251),
		color.color_from_rgb(133, 220, 130),
		color.color_from_rgb(136, 196, 95),
		color.color_from_rgb(54, 198, 146),
		color.color_from_rgb(126, 218, 200),
		color.color_from_rgb(128, 160, 255),
		color.color_from_rgb(138, 175, 255),
		color.color_from_rgb(116, 178, 255),
		color.color_from_rgb(173, 205, 243),
		color.color_from_rgb(174, 129, 255),
		color.color_from_rgb(207, 135, 238),
		color.color_from_rgb(230, 94, 114),
		color.color_from_rgb(255, 81, 137),
		color.color_from_rgb(255, 84, 84),
		color.color_from_rgb(244, 175, 111),
		color.color_from_rgb(205, 172, 252),
	]
	mut colors := []color.Color{}
	for base in base_colors {
		colors << if theme { base } else { base.darken(0.3) }
	}
	return colors
}
...
fn too_close(candidate color.Color, others []color.Color, min_delta f64) bool {
	for other in others {
		if color.delta_e_cie76(candidate.to_lab(), other.to_lab()) < min_delta {
			return true
		}
	}
	return false
...
pub fn ensure_distinct(colors []color.Color, min_delta f64) []color.Color {
	mut out := []color.Color{}
	for c in colors {
		mut candidate := c
		base := c.to_hsl()
		...
		for attempt := 1; attempt <= 24; attempt++ {
			...
			if base.s > 0.15 {
				...
				candidate = color.color_from_hsl(base.h + 47.0 * f64(attempt), base.s, base.l)
			} else {
				...
				candidate = color.color_from_hsl(base.h, base.s, lightness)
			}
		}
		...
		if too_close(candidate, out, min_delta) {
			...
			candidate = color.color_from_hsl(base.h, base.s, 0.12 + 0.76 * (f64(out.len) / span))
		}
		out << candidate
	}
	return out
...
pub fn sort_palette(mut palette []color.Color, mode string) {
	match mode {
...
pub fn adjust_palette(palette []color.Color, scheme &config.Scheme) []color.Color {
	...
	for c in palette {
		...
		if scheme.saturation != 0.0 {
			s = color.clamp01(s * (1.0 + scheme.saturation))
		}
		if scheme.illumination != 0.0 {
			l = color.clamp01(l + scheme.illumination)
		}
		out << color.color_from_hsl(h, s, l)
	}
	...
	if scheme.blend > 0.0 && out.len > 1 {
		anchor := out[0]
		for i := 1; i < out.len; i++ {
			out[i] = out[i].mix_lab(anchor, color.clamp01(scheme.blend))
		}
	}
	return out
}
...
pub fn get_all_colors(mut scheme config.Scheme) []color.Color {
	...
	for c in scheme.pigments {
		palette << color.color_from_hex(c)
	}
	...
	mut black := color.color_from_rgb(0, 0, 0)
	mut white := color.color_from_rgb(255, 255, 255)
	if !theme {
		white = color.color_from_rgb(0, 0, 0)
		black = color.color_from_rgb(255, 255, 255)
	}
	...
	for _ in 0 .. 10 {
		...
		colors << gen_shades([col0, color.color_from_hsl(hue, saturation, lightness), col15], 12)
	}
	...
	if scheme.norandom {
		for _ in 0 .. 6 {
			...
			saturation := 0.2 + 0.6 * rand.f64()
			lightness := 0.3 + 0.4 * rand.f64()
			colors << gen_shades([col0, color.color_from_hsl(hue, saturation, lightness),
				col15], 12)
		}
	} else {
		lightish := gradients[2]
		darkish := gradients[21]
		colors << gen_shades([lightish, color.color_from_rgb(255, 0, 0), darkish], 24)
		colors << gen_shades([lightish, color.color_from_rgb(0, 255, 0), darkish], 24)
		colors << gen_shades([lightish, color.color_from_rgb(0, 0, 255), darkish], 24)
	}
...
pub fn raise_contrast(c color.Color, background color.Color, min_ratio f64) color.Color {
	...
	ratio := fn [bg] (candidate color.Color) f64 {
		return color.contrast_ratio(candidate.quantised(), bg)
	}
	...
	mut limit := preferred
	if ratio(color.color_from_hsl(hsl.h, hsl.s, preferred)) < min_ratio {
		if ratio(color.color_from_hsl(hsl.h, hsl.s, opposite)) >= min_ratio {
			limit = opposite
		} else {
			...
			toward_preferred := ratio(color.color_from_hsl(hsl.h, hsl.s, preferred))
			toward_opposite := ratio(color.color_from_hsl(hsl.h, hsl.s, opposite))
			best := if toward_preferred >= toward_opposite { preferred } else { opposite }
			return color.color_from_hsl(hsl.h, hsl.s, best)
		}
	}
	...
	for _ in 0 .. 24 {
		mid := (near + far) / 2.0
		if ratio(color.color_from_hsl(hsl.h, hsl.s, mid)) >= min_ratio {
			far = mid
		} else {
			near = mid
		}
	}
	return color.color_from_hsl(hsl.h, hsl.s, far)
}
...
pub fn enforce_contrast(colors []color.Color, min_ratio f64) []color.Color {
...
fn separate_group(mut out []color.Color, group []int, background color.Color, min_ratio f64) {
	mut placed := []color.Color{}
	for index in group {
		mut c := out[index]
		if too_close(c, placed, 10.0) {
			...
			for target in [10.0, 6.0, 3.0, 1.5] {
				...
				for step in 1 .. 33 {
					...
					candidate := color.color_from_hsl(hsl.h, hsl.s, lightness)
					if !too_close(candidate, placed, target)
						&& color.contrast_ratio(candidate.quantised(), background.quantised()) >= min_ratio {
						c = candidate
						found = true
						break
					}
